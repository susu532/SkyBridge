      ...
      if (container) {
        ...
        for (let i = 0; i < entityTags.length; i++) {
          ...
          if (!el) {
            ...
            innerDiv.className = `px-1 py-0.25 md:px-3 md:py-1 rounded border flex items-center gap-0.5 md:gap-2 whitespace-nowrap mc-font text-[7px] md:text-[16px] shadow-lg ${isPlayer ? 'bg-black/40 border-white/10' : 'bg-black/70 border-white/20'}`;
            
            if (!tag.isPassive && !isPlayer && tag.type !== 'Morvane') {
              const lvSpan = document.createElement('span');
              lvSpan.className = "text-[#FFFF55] font-bold text-[8px] md:text-[18px]";
